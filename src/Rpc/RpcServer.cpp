...
// Copyright (c) 2016, The Karbo developers
...
bool RpcServer::masternode_check_incoming_tx(const BinaryArray& tx_blob) {
	...
	if (amount != 0) {
		logger(INFO) << "Masternode received relayed transaction fee: " << m_core.currency().formatAmount(amount) << " BR";
		return true;
	}
	return false;
}
...
bool RpcServer::f_on_block_json(const F_COMMAND_RPC_GET_BLOCK_DETAILS::request& req, F_COMMAND_RPC_GET_BLOCK_DETAILS::response& res) {
  ...
  if (!m_core.getBlockReward(res.block.major_version, res.block.sizeMedian, 0, prevBlockGeneratedCoins, 0, maxReward, emissionChange, height)) {
    return false;
  }
  if (!m_core.getBlockReward(res.block.major_version, res.block.sizeMedian, res.block.transactionsCumulativeSize, prevBlockGeneratedCoins, 0, currentReward, emissionChange, height)) {
    return false;
  }
