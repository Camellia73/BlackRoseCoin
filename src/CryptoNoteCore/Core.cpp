...
bool core::fillBlockDetails(const Block &block, BlockDetails2& blockDetails) {
  ...
  uint32_t height = 0;
  if (!getBlockReward(block.majorVersion, blockDetails.sizeMedian, 0, prevBlockGeneratedCoins, 0, maxReward, emissionChange, height)) {
    return false;
  }

  if (!getBlockReward(block.majorVersion, blockDetails.sizeMedian, blockDetails.transactionsCumulativeSize, prevBlockGeneratedCoins, 0, currentReward, emissionChange, height)) {
    return false;
  }
